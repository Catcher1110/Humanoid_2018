...
#include <Atlas_Controller/ContactSet/DoubleContact.hpp>
#include <Atlas_Controller/Atlas_StateProvider.hpp>
...
void ConfigBodyCtrl::_jpos_task_setup(){
    ...
    dynacore::Vector pos_des(atlas::num_qdot); pos_des.setZero();
    dynacore::Vector vel_des(atlas::num_qdot); vel_des.setZero();
    dynacore::Vector acc_des(atlas::num_qdot); acc_des.setZero();
    ...
    for (int i(0); i<atlas::num_act_joint; ++i){
        ...
        pos_des[atlas::num_virtual + i] = des_jpos_[i];
        des_jvel_[i] = 0.;
    }
...
void ConfigBodyCtrl::FirstVisit(){
    ...
    ini_body_height_ = sp_->Q_[atlas_joint::virtual_Z];
}
