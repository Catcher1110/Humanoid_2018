    ...
    initial_traj_mix_ratio_(0.6)
{
    ...
    if(swing_foot == valkyrie_link::leftFoot) {
        single_contact_ = new SingleContact(robot, valkyrie_link::rightFoot); 
    ...
    for(size_t i = 0; i < valkyrie::num_leg_joint; i++){
...
void JPosTrajPlanningCtrl::_task_setup(){
    ...
    for (int i(0); i<valkyrie::num_act_joint; ++i){
        ...
        vel_des[valkyrie::num_virtual + i] = qdot_cmd[valkyrie::num_virtual + i];
        acc_des[valkyrie::num_virtual + i] = qddot_cmd[valkyrie::num_virtual + i];

        des_jpos_[i] = pos_des[valkyrie::num_virtual + i];
        des_jvel_[i] = vel_des[valkyrie::num_virtual + i];
    }
...
void JPosTrajPlanningCtrl::FirstVisit(){
    ...
    ini_swing_leg_config_ = sp_->Q_.segment(swing_leg_jidx_, valkyrie::num_leg_joint);
    robot_sys_->getPos(swing_foot_, ini_foot_pos_);
    dynacore::Vector zero(valkyrie::num_leg_joint);
    zero.setZero();
    ...
    dynacore::Vector mid_mix_pt_pos(valkyrie::num_leg_joint);
    dynacore::Vector mid_mix_pt_vel(valkyrie::num_leg_joint);
    dynacore::Vector mid_mix_pt_acc(valkyrie::num_leg_joint);
